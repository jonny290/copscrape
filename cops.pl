...
my $isrunning = `ps aux | grep cops.pl | wc -l`;
if ($isrunning > 4) {exit 0;}
...
until (($log[0] =~ /You are viewing all calls for:/) or (scalar(@log) == 0)) {
...
$today[1] =~ s/,//g;
our $convdate = $today[2]."-".sprintf("%02d",$today[0])."-".sprintf("%02d",$today[1]);
...
#	print $convdate." ".$itemtime." | " .sprintf("%-40s",$itemdesc)." | " .sprintf("%-40s",$address)."\n";
	$loghash{$convdate}{$itemtime}{$address}{$itemdesc} = 1 unless (exists($loghash{$convdate}{$itemtime}{$address}{$itemdesc}));
}
		...
		foreach my $time (keys %{$loghash{$date}}) {
			foreach my $add (keys %{$loghash{$date}{$time}}) {
				foreach my $desc (keys %{$loghash{$date}{$time}{$add}}) {
					...
					my $tst = $dbic->do("select * from incidents where md5 = '$md5'");
					unless ($tst == 1) { my ($sitela, $sitelo) = &geoloc($add);my $exec = $dbic->do("insert into incidents values ('$md5','$ts','$add','$desc','$sitela','$sitelo',false)");};
				}
			}
		}
	}
...
#print Dumper(\%loghash);

#store \%loghash,'./loghash'; 
